...
--
-- Name: check_comment_limit(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.check_comment_limit() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    IF (SELECT COUNT(*) FROM comments WHERE post_id = NEW.post_id AND user_id = NEW.user_id) >= 10 THEN
        RAISE EXCEPTION 'User has exceeded the maximum comment limit for this post';
    END IF;
    RETURN NEW;
END;
$$;
...
ALTER FUNCTION public.check_comment_limit() OWNER TO postgres;
